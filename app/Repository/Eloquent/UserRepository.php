    ...
    implements UserRepositoryInterface
{
    ...
    public function getSocialiteUserByProviderId($user, $provider): User
    {
        ...
        $authUser = $this->model->where('provider_id', $user->id)->where('provider', strtoupper($provider))->first();
        ...
        if ($user->email) {
            $authUser = $this->model->where('email', $user->email)->first();
            ...
            if ($authUser) {
                ...
                $authUser->update([
                    ...
                    'provider_id' => $user->id,
                    'email_verified_at' => $authUser->email_verified_at ?? $this->carbon->now(),
                ]);
        ...
        $authUser = $this->model->create([
            ...
            'provider' => strtoupper($provider),
            'provider_id' => $user->id,
            'password' => Hash::make(uniqid()), // Generate a random password for social users
        ]);
